...
class PostsMaster {
    private dbController: any; // Adjust the type according to your DBController type
     ...
     */
    async createPost({ property_id, creator_id, content, replied_to = "" }: { property_id: string; creator_id: string; content: string; replied_to?: string }) {
        try {
     ...
     */
    async getUserPosts(creator_id: string) {
        try {
     ...
     */
    async getPropertyPosts(property_id: string) {
        try {
